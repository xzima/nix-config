{ flake, inputs, hostName, perSystem, config, modulesPath, pkgs, lib, ... }:
{
  ...
  hardware = {
    ...
    nvidia = {
      ...
      package = config.boot.kernelPackages.nvidiaPackages.production; # https://github.com/NixOS/nixpkgs/blob/nixos-25.11/pkgs/os-specific/linux/nvidia-x11/default.nix#L74
    };
  };
